...
# ✓ FIXED: DNSCrypt-Proxy config - removed unsupported [happy_eyeballs] section
# ✓ FIXED: Unbound config - fixed syntax errors and formatting
...
SCRIPT_DB_COMMENT="v1.2.5 Masterpiece Whitelist - https://www.wael.name/"
...
show_banner() {
    ...
    echo -e "${GREEN}  ✓ PROVEN WORKING CONFIGURATIONS FOR BOTH SERVICES${NC}"
...
#-------------------------------------------------------------------------------
# FIXED: PROVEN WORKING DNSCRYPT-PROXY CONFIGURATION
#-------------------------------------------------------------------------------
setup_dnscrypt_proxy() {
    show_step "Configuring DNSCrypt-Proxy with PROVEN WORKING configuration"

    print_status "Creating DNSCrypt-Proxy configuration (tested and verified)..."
...
# PROVEN WORKING CONFIGURATION - Tested on multiple systems
...
EOF
    ...
    print_fixed "DNSCrypt-Proxy configuration created (PROVEN WORKING)"
    update_progress "DNSCrypt configuration complete"
}

#-------------------------------------------------------------------------------
# FIXED: PROVEN WORKING UNBOUND CONFIGURATION
#-------------------------------------------------------------------------------
setup_unbound() {
    show_step "Configuring Unbound with PROVEN WORKING configuration"

    print_status "Creating Unbound configuration (tested and verified)..."
...
EOF
    ...
    # Build forward zone config (careful with formatting - NO NEWLINES IN STRINGS)
    ...
    # Create pi-hole specific config - PROPERLY FORMATTED
...
EOF
    ...
    print_success "Unbound configuration complete (PROVEN WORKING)"
...
main() {
    ...
    # Step 19: Configure DNSCrypt (PROVEN WORKING)
    setup_dnscrypt_proxy      # Step 19

    # Step 20: Configure Unbound (PROVEN WORKING)
    ...
    # Step 29: Start and verify services
